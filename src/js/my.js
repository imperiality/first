...
/*
 目录：
    33行    getWinSize()：获取屏幕宽高.无参数，返回一个对象，width属性获取屏幕的宽度，height属性获取屏幕的高度
    48行    getID(eleIDName)：通过ID获取元素，参数是元素的ID名  返回获取的元素
    57行    getEleToWin(ele)：1个参数，元素节点 获取元素距离屏幕边缘的位置 返回一个对象 包含left和top
    82行    getWinScroll()：没有参数，获取浏览器滚动条的位置，返回一个对象，包含top和left属性。 
    98行     setWinScroll(n,m): 设置浏览器滚动条位置，两个参数，第一个是横向滚动条， 
                                第二个是竖向滚动条，如果值传递一个，代表横向。 
    113行    getDocSize()：无参数，获取文档的宽高，返回一个对象，包含width和height属性。 
    126行    getPreSibling(ele):获取ele元素的上一个兄弟元素 一个参数。   
    153行   getfirChild(ele):获取ele元素的第一个子元素节点。
    162行    getlastChild(ele):获取ele元素的最后一个子节点。 
    174行    prependChild(paren,ele):两个参数，父节点和被插入的节点,   
                                        功能:在元素的子元素的最前边插入节点方法。
    188行    insetAfter(newele,oldele):两个参数，新节点和旧节点，在某个节点的后边插入元素。   
    202行    remove（ele）  一个参数  删除某一个元素。   
    215行    empty（ele） 一个参数  清空当前元素的所有子节点。
    224行    类名ID标签name获取元素方法。            
    241行    document.createAttribute(name)创建属性节点。                                  
    248行    pageX和pageY的封装
    262行    删除和添加DOM2事件兼容写法
* */
+function (w) {
    w.my = {};
    ...
    *   两个参数 分别是父节点和被插入的节点   功能：在元素的子元素的最前边插入节点方法
    * */
    //在元素的最前边插入节点方法封装
    ...
    * */
    ...
    //创建属性节点
    my.dc_attr = function (name) {
        return document.createAttribute(name)
    } 

    // pageX和pageY的封装
    my.getEventPage = function (e) {
        if (e.pageX){
            return {
                x:e.pageX,
                y:e.pageY
    ...
    //删除和添加DOM2事件兼容
    my.removeEvent = function (ele,type,f) {
    ...
    my.addEvent = function (ele,type,f,boo){
        if (ele.addEventListener){
            ele.addEventListener(type,f,boo || false);
